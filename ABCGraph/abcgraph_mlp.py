...
class ABCGraphMLP(object):
    def __init__(self, bipartite_graph_data_loader, args, device, rank=-1, dataset="cora"):
        """For mlp layer, we can define any output dimension from GCN layer"""
        self.rank = rank
    ...
    def __save_embedding_to_file(self, gcn_merge_output, node_id_list):
        ...
        output_folder = "./out/abcgraph-mlp/" + str(self.dataset)
        if self.rank != -1:
            output_folder = "/mnt/shared/home/bipartite-graph-learning/out/abcgraph-mlp/" + self.dataset + "/" + str(
                self.rank)
