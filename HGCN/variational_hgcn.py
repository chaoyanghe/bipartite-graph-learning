...
"""
Three steps for Variational HGCN.
1. Aggregate features from set V to U (explicit). 
2. Aggregate features from set U to V (implicit).
3. Align the generated features with original features in V using VAE.
The above processes will be considered as one layer. 
The latent vector in VAE will be the node embedding for set V (any dimension).
"""
class VariationalGCNLayer(object):
    def __init__(self, bipartite_graph_data_loader, args, device, layerInfo, rank):
        self.rank = rank
        ...
        vae_hidfeat = args.vae_hidfeat
        gcn_output_dim = args.gcn_output_dim
        # TODO: Here I need to define a list to store the layers information, than build the initialize each GCN layer.
        layer_infos = [layerInfo(vae_hidfeat, u_attr_dimensions, v_attr_dimensions, v_attr_dimensions, gcn_output_dim),
                       layerInfo(vae_hidfeat, vae_hidfeat, v_attr_dimensions, v_attr_dimensions, gcn_output_dim)]

        # the explicit steps output must have the same dimensions as set U, due to the features aggregation
